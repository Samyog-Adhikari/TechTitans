...
export default function CollectorComplaintPage() {
  const { user } = useAuth()
  ...
  const loadComplaints = async () => {
    ...
    try {
      ...
      console.error("Error loading collector complaints:", err)
    } finally {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.error("Error submitting collector issue:", err)
    ...
    <div className="space-y-8 max-w-4xl mx-auto animate-in fade-in-50">
      <div>
        <h1 className="text-2xl sm:text-3xl font-extrabold tracking-tight text-slate-900 dark:text-white">
          Collector Support & Dispute Submission
        </h1>
        <p className="text-xs sm:text-sm text-slate-500 mt-1">
          Report issues with household availability, address discrepancies, contaminated scrap, or digital scales
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-8 items-start">
        ...
        <Card className="lg:col-span-3 rounded-2xl border-slate-200/80 dark:border-border shadow-sm bg-white dark:bg-card p-6 sm:p-7">
          <CardHeader className="p-0 pb-5">
            ...
            <CardDescription className="text-xs">
              KabadMandu support reviews and responds to collector issues within 24 hours.
            </CardDescription>
          </CardHeader>
          ...
          {success && (
            <div className="p-4 rounded-xl bg-emerald-50 dark:bg-emerald-950/50 border border-emerald-200 dark:border-emerald-800 text-emerald-800 dark:text-emerald-300 text-xs font-semibold mb-4 flex items-center gap-2">
              ...
              <span>Your issue has been reported to administration.</span>
            </div>
          )}
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="space-y-1.5">
              ...
              <Input
                id="subject"
                required
                placeholder="e.g. Household unreachable / Contaminated scrap / Address error"
                value={subject}
            ...
            <div className="space-y-1.5">
              ...
              <textarea
                ...
                rows={4}
                required
                placeholder="Provide pickup address, time attempted, customer contact attempts, or description of the issue..."
                value={description}
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          {fetchLoading ? (
            ...
            <EmptyState icon={MessageSquare} title="No issues submitted" description="You haven't raised any complaints or support tickets yet. Use the form above to report an issue." />
          ) : (
            <div className="space-y-3">
              {complaints.map((c) => (
                <Card
                ...
                >
                  ...
                  {c.resolution_note && (
                    <div className="p-2.5 rounded-lg bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-200/80 dark:border-emerald-900 text-[11px] text-emerald-900 dark:text-emerald-300">
                      <strong>Admin Resolution:</strong> {c.resolution_note}
                    </div>
                  )}
